...
import (
	...
	domain "github.com/anthropics/agentmesh/backend/internal/domain/devpod"
)
...
type MockSettingsService struct {
	mu       sync.RWMutex
	settings map[int64]*domain.UserDevPodSettings
	err      error
}
...
func NewMockSettingsService() *MockSettingsService {
	return &MockSettingsService{
		settings: make(map[int64]*domain.UserDevPodSettings),
	}
}
...
func (m *MockSettingsService) AddSettings(settings *domain.UserDevPodSettings) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockSettingsService) GetUserSettings(ctx context.Context, userID int64) (*domain.UserDevPodSettings, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	fontSize := 14
	theme := "dark"
	return &domain.UserDevPodSettings{
		UserID:             userID,
...
func (m *MockSettingsService) UpdateUserSettings(ctx context.Context, userID int64, updates *UserSettingsUpdate) (*domain.UserDevPodSettings, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	if !ok {
		settings = &domain.UserDevPodSettings{UserID: userID}
	}
