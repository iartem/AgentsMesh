...
func setupSettingsTestDB(t *testing.T) *gorm.DB {
	...
	err = db.Exec(`
		CREATE TABLE IF NOT EXISTS user_devpod_settings (
	...
	`).Error
	if err != nil {
		t.Fatalf("failed to create user_devpod_settings table: %v", err)
	}
...
func TestGetUserSettings_NewUser(t *testing.T) {
	...
	var savedSettings devpod.UserDevPodSettings
...
func TestGetUserSettings_ExistingUser(t *testing.T) {
	...
	script := "echo hello"
	existing := &devpod.UserDevPodSettings{
		UserID:             2,
...
func TestDeleteUserSettings(t *testing.T) {
	...
	// Verify deleted
	var count int64
	db.Model(&devpod.UserDevPodSettings{}).Where("user_id = ?", 1).Count(&count)
	if count != 0 {
