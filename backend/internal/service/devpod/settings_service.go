...
var (
	ErrSettingsNotFound = errors.New("user DevPod settings not found")
)

// SettingsService handles user DevPod settings operations
...
// GetUserSettings returns the DevPod settings for a user
// Creates default settings if none exist
func (s *SettingsService) GetUserSettings(ctx context.Context, userID int64) (*devpod.UserDevPodSettings, error) {
	var settings devpod.UserDevPodSettings
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			settings = devpod.UserDevPodSettings{
				UserID:             userID,
...
// UpdateUserSettings updates the DevPod settings for a user
func (s *SettingsService) UpdateUserSettings(ctx context.Context, userID int64, updates *UserSettingsUpdate) (*devpod.UserDevPodSettings, error) {
...
// DeleteUserSettings removes DevPod settings for a user
func (s *SettingsService) DeleteUserSettings(ctx context.Context, userID int64) error {
	return s.db.WithContext(ctx).
		Where("user_id = ?", userID).
		Delete(&devpod.UserDevPodSettings{}).Error
}
